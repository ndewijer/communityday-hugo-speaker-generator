...
def test_retry_logic_verbose():
    ...
    if hasattr(processor, "linkedin_extractor") and processor.linkedin_extractor:
        ...
        print(
            f'   • Authentication: {"✅ Authenticated" if processor.linkedin_extractor.is_authenticated() else "❌ Not authenticated"}'
        )
        ...
        if processor.linkedin_extractor.is_authenticated():
            ...
            print(
                f'   • Auth test result: {"✅ Working" if auth_test else "❌ Failed"}'
            )

        print(
            f"   • Available strategies: {processor.linkedin_extractor.get_extraction_strategies()}"
        )
    else:
    ...
    for i, retry_item in enumerate(processor.retry_queue, 1):
        ...
        if email in speakers:
            ...
            print(
                f'      • Custom photo: {speaker_data.get("custom_photo_url", "N/A")}'
            )
            ...
            if linkedin_url:
                ...
                try:
                    ...
                    if (
                        hasattr(processor, "linkedin_extractor")
                        and processor.linkedin_extractor
                    ):
                        normalized_url = (
                            processor.linkedin_extractor.normalize_linkedin_url(
                                linkedin_url
                            )
                        )
                        ...
                        image_url = (
                            processor.linkedin_extractor.extract_profile_image_url(
                                linkedin_url
                            )
                        )
                        print(
                            f'      • Extracted image URL: {image_url if image_url else "❌ None found"}'
                        )
                        ...
                        if image_url:
                            ...
                            try:
                                ...
                                print(
                                    f"      • Image URL status: {response.status_code}"
                                )
                            except Exception as e:
                        ...
                        print(
                            f'      • Basic extraction result: {image_url if image_url else "❌ None found"}'
                        )
                ...
                if os.path.exists(img_path):
                    ...
                    if os.path.exists(default_path):
                        ...
                        if file_size == default_size:
                            print(
                                f"   📁 File appears to be default image (same size as {default_path})"
                            )
                        else:
                            print(
                                f"   📁 File appears to be custom image (different size from default)"
                            )
                else:
